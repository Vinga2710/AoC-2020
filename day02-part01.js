...
const makrama = /(\d+)-(\d+) (\w): (\S+)/;
// console.log(list.map((pass) => makrama.test(pass)));
// console.log(list.map((pass) => pass.match(makrama)));

const valid = list.filter((pass) => {
  const part = pass.match(makrama);
  let min = part[1];
  let max = part[2];
  min = Number(min);
  max = Number(max);
  const letter = part[3];
  const password = part[4];
  console.log([min, max, letter, password]);
  const numberOf = [...password].filter((char) => char === letter).length;
  return numberOf >= min && numberOf <= max;
}).length;
console.log(valid);
